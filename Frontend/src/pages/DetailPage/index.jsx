...
function DetailPage() {
    ...
    return (
        <Container>
            <Box>
                <Button variant="outlined" sx={{ margin: '10px 0px', color: 'navy' }} onClick={() => navigation('edit', { state: { postData: data[0] } })}>
                    edit
                </Button>
            </Box>
            {data[0] && (
                <Box sx={{ display: 'flex', padding: '20px 0px', justifyContent: 'space-between' }}>
                    <Box sx={{ overflow: 'hidden', width: '782px' }}>
                        <img style={{width:'500px', height:'500px'}} src={data[0].images[0]} alt="hero"/>
